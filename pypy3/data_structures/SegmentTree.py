class SegmentTree:
    ...
    def _build_seg_tree(self, tree_index: int, left_index: int, right_index: int):
        if left_index > right_index:
            ...
            self.segTree[tree_index] = max(0, self.segTree[2 * tree_index + 1]) + max(
                0, self.segTree[2 * tree_index + 2]
            )
    ...
    def update_seg_tree(
        ...
        if left_index == right_index:
            ...
            mid = (left_index + right_index) // 2
            if target_index <= mid:
                self.update_seg_tree(
                    2 * tree_index + 1, left_index, mid, target_index, new_value
                )
            else:
                self.update_seg_tree(
                    2 * tree_index + 2, mid + 1, right_index, target_index, new_value
                )
            self.segTree[tree_index] = (
                self.segTree[2 * tree_index + 1] + self.segTree[2 * tree_index + 2]
            )
    ...
    def find_range_sum(
        ...
        left_index: int,
        right_index: int,
        left_query_index: int,
        right_query_index: int,
    ) -> int:
